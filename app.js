...
async function handler(req,res){
  ...
  if (url.pathname === '/api/bootstrap' && req.method === 'GET') {
    ...
    return send(res,200,{ ok:true, ...base, todayKey, completionByCharacter, month:{ today:`${y}-${String(m).padStart(2,'0')}-${String(d).padStart(2,'0')}`, monthEndsAt, daysRemaining } });
  }
  ...
  if (url.pathname === '/api/register' && req.method === 'POST') {
    try {
      ...
      if (!/^\d{4}$/.test(password)) return send(res,400,{ok:false,error:'PIN must be exactly 4 digits.'});
